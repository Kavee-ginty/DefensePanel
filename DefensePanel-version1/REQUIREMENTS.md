...
| History shows **another user's** sessions | Run [`supabase/pitch_sessions_rls_hardening.sql`](supabase/pitch_sessions_rls_hardening.sql) in Supabase (RLS + policies). Hard refresh after **Sign out**, then sign in again. Client code filters by `user_id` as defense-in-depth. |
